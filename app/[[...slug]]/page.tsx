...
const CHROME_EXTENSION_URL = "https://chromewebstore.google.com/detail/ninjaswiper/jppnpkfbjilbcfbkiajdbgpcioojeihl?pli=1";
...
const plans = [
  {
    ...
    cta: "Start free",
    featured: false,
    perks: ["200 trial swipes", "Custom swipe timing", "Passport city cycling", "Random breaks"],
  },
  {
    name: "Premium",
    price: "$9",
    ...
    cta: "Go premium",
    featured: true,
    perks: ["Unlimited swipes", "All extension settings", "Same API key after upgrade", "Cancel anytime"],
  },
];

const featureCards = [
  { icon: "zap", title: "Set your pace", body: "Choose the time between swipes and the percentage of profiles to Like." },
  { icon: "globe", title: "Change locations", body: "Add a list of Passport cities for the extension to try when profiles run out." },
  { icon: "shield", title: "Take breaks", body: "Turn on optional breaks and see when the extension is paused." },
  { icon: "key", title: "Start free", body: "Create an account for your API key and try 200 swipes before subscribing." },
];
...
const faqs = [
  ...
  ["What happens after 200 swipes?", "You can upgrade to Premium for $9/month to keep swiping with the same API key."],
  ["How do I get started?", "Install NinjaSwiper from the Chrome Web Store, create a free account, then copy the API key from your dashboard into the extension."],
];
...
function getBillingActionLabel(isPaid: boolean) {
  return isPaid ? "Manage billing" : "Upgrade for $9";
}
...
function getAccountStatus(isPaid: boolean, swipesUsed: number, freeLimit = 200) {
  if (isPaid) {
    return { label: "Premium active", tone: "premium", swipesLeft: "Unlimited", actionText: "Manage billing", plan: "Premium" };
  }
...
if (typeof console !== "undefined") {
  ...
  console.assert(getBillingActionLabel(true) === "Manage billing", "Expected paid users to see billing action.");
...
function Icon({ name, className = "h-5 w-5" }: { name: string; className?: string }) {
  const common: React.SVGProps<SVGSVGElement> = { className, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true };
...
function DashboardLayout({ onNavigate }: { onNavigate: (route: RouteName) => void }) {
  ...
  async function handleCopyKey() {
    ...
    if (await copyText(profile.api_key)) {
      setCopied(true);
  ...
  return (
    <main className="min-h-screen bg-[#fff7f8] px-6 py-6 text-zinc-950">
      ...
      <section className="mx-auto max-w-7xl py-10 md:py-16">
        {loading && <Card className="rounded-[2rem] border-rose-100 bg-white shadow-xl shadow-rose-200/30"><CardContent className="p-8 text-center"><Badge>Loading dashboard</Badge><h1 className="mt-5 text-4xl font-black">Checking your account…</h1><p className="mt-3 text-zinc-600">Your dashboard will be ready in a moment.</p></CardContent></Card>}
          ...
          <div>
            <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-lg leading-8 text-zinc-600">Your API key and swipe allowance are below.</p>
              </div>
            ...
            <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
              <Card className="rounded-[2rem] border-rose-100 bg-white shadow-xl shadow-rose-200/30">
                <CardContent className="p-6 md:p-8">
                  <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start">
                    <div><p className="text-sm font-black uppercase tracking-[0.22em] text-[#ff5864]">API key</p><h2 className="mt-2 text-3xl font-black">Connect your extension</h2><p className="mt-2 max-w-xl leading-7 text-zinc-600">Install the Chrome extension, then paste your key into its API key field.</p><a href={CHROME_EXTENSION_URL} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex font-bold text-[#d92350] underline underline-offset-4 hover:text-[#a7193b]">Install NinjaSwiper for Chrome ↗</a></div>
              ...
              <div className="grid gap-5">
                <Card className="rounded-[2rem] border-rose-100 bg-zinc-950 text-white shadow-xl shadow-zinc-300/40"><CardContent className="p-6 md:p-8"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-rose-300">Subscription</p><h2 className="mt-2 text-3xl font-black">{isPaid ? "Premium is active" : account.tone === "blocked" ? "Trial ended" : "Upgrade when ready"}</h2></div><Icon name={isPaid ? "shield" : "zap"} className="h-8 w-8 text-rose-300" /></div><p className="mt-4 leading-7 text-white/65">{isPaid ? "Manage or cancel your subscription from the billing page." : account.tone === "blocked" ? "You’ve used your 200 free swipes. Upgrade to keep using the extension." : "You have 200 free swipes. Upgrade whenever you’re ready for unlimited swiping."}</p><Button type="button" onClick={() => onNavigate("payment")} className="mt-6 h-12 w-full rounded-2xl bg-white font-black text-zinc-950 hover:bg-rose-50">{getBillingActionLabel(isPaid)}</Button></CardContent></Card>
                <Card className="rounded-[2rem] border-rose-100 bg-white shadow-sm"><CardContent className="p-6 md:p-8"><p className="text-sm font-black uppercase tracking-[0.22em] text-[#ff5864]">Your account</p><div className="mt-5 space-y-4 text-sm font-semibold text-zinc-600"><div className="flex items-start gap-3"><Icon name="check" className="mt-0.5 h-5 w-5 shrink-0 text-[#ff5864]" /> Email: {profile?.email}</div><div className="flex items-start gap-3"><Icon name="check" className="mt-0.5 h-5 w-5 shrink-0 text-[#ff5864]" /> Plan: {isPaid ? "Premium" : "Free trial"}</div></div></CardContent></Card>
              </div>
            </div>
          </div>
        )}
    ...
    <main className="min-h-screen bg-[#fff7f8] px-6 py-6 text-zinc-950">
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          ...
          <p className="mt-5 max-w-xl text-lg leading-8 text-zinc-600">Premium removes the 200-swipe trial limit. Your current API key stays the same.</p>
...
function ContactLayout({ onNavigate }: { onNavigate: (route: RouteName) => void }) {
  return (
    <main className="min-h-screen bg-[#fff7f8] px-6 py-6 text-zinc-950">
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}><Badge>Contact us</Badge><h1 className="mt-6 text-5xl font-black leading-[0.95] tracking-tight md:text-6xl">Need help with NinjaSwiper?</h1><p className="mt-5 max-w-xl text-lg leading-8 text-zinc-600">Reach out for account, billing, API key, extension, or setup questions. Email us and we’ll help you get back on track.</p><div className="mt-8 grid max-w-md gap-3 text-sm font-bold text-zinc-500"><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />API key and account help</span><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />Billing and subscription support</span><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />Extension troubleshooting</span></div></motion.div>
...
function ForgotPasswordLayout({ onNavigate }: { onNavigate: (route: RouteName) => void }) {
  ...
  return (
    <main className="min-h-screen bg-[#fff7f8] px-6 py-6 text-zinc-950">
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}><Badge>Recover access</Badge><h1 className="mt-6 text-5xl font-black leading-[0.95] tracking-tight md:text-6xl">Reset your password.</h1><p className="mt-5 max-w-xl text-lg leading-8 text-zinc-600">Enter the email connected to your NinjaSwiper account. We’ll email you a link to reset your password.</p></motion.div>
...
function UpdatePasswordLayout({ onNavigate }: { onNavigate: (route: RouteName) => void }) {
  ...
  return (
    <main className="min-h-screen bg-[#fff7f8] px-6 py-6 text-zinc-950">
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-10 py-16 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}><Badge>Choose a new password</Badge><h1 className="mt-6 text-5xl font-black leading-[0.95] tracking-tight md:text-6xl">Create your new password.</h1><p className="mt-5 max-w-xl text-lg leading-8 text-zinc-600">Enter a new password for your NinjaSwiper account.</p></motion.div>
    ...
    <main className="min-h-screen overflow-hidden bg-[#fff7f8] text-zinc-950">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-20 pt-10 lg:grid-cols-[1.05fr_0.95fr] lg:pb-28 lg:pt-16">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <h1 className="mt-6 max-w-3xl text-5xl font-black leading-[0.95] tracking-tight text-zinc-950 md:text-7xl">Let NinjaSwiper handle the swiping.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-zinc-600">Set your swipe speed and Like percentage, add Passport cities, and take breaks when you want. Start with 200 free swipes.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button type="button" onClick={() => navigate("signup")} className="h-14 rounded-2xl bg-[#ff5864] px-7 text-base font-black text-white shadow-lg shadow-rose-300/50 hover:bg-[#ff2a4e]">Start free trial <Icon name="arrow-right" className="ml-2 h-5 w-5" /></Button><a href={CHROME_EXTENSION_URL} target="_blank" rel="noopener noreferrer" className="inline-flex h-14 items-center justify-center rounded-2xl border border-rose-200 bg-white px-7 text-base font-black text-zinc-950 transition hover:bg-rose-50">Install for Chrome ↗</a></div>
          <div className="mt-8 flex max-w-xl flex-wrap gap-x-6 gap-y-3 text-sm font-bold text-zinc-500"><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />200 free swipes</span><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />Adjustable timing</span><span className="inline-flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-[#ff5864]" />Passport cities</span></div>
        </motion.div>
        <SwipeDemo />
      </section>

      <section id="features" className="mx-auto max-w-7xl px-6 py-16"><div className="max-w-2xl"><p className="text-sm font-black uppercase tracking-[0.25em] text-[#ff5864]">Features</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Swiping settings that fit your routine.</h2><p className="mt-4 text-lg leading-8 text-zinc-600">Choose your pace, add the places you want to browse, and let NinjaSwiper take it from there.</p></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{featureCards.map((feature) => <FeatureCard key={feature.title} icon={feature.icon} title={feature.title} body={feature.body} />)}</div></section>

      <section id="pricing" className="mx-auto max-w-7xl px-6 py-16"><div className="rounded-[2rem] bg-zinc-950 p-6 text-white shadow-2xl shadow-zinc-300/50 md:p-10"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="text-sm font-black uppercase tracking-[0.25em] text-rose-300">Pricing</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Start free. Upgrade when ready.</h2></div><p className="max-w-md leading-7 text-white/60">Try 200 swipes for free. Premium gives you unlimited swipes for $9 a month.</p></div><div className="mt-10 grid gap-5 lg:grid-cols-2">{plans.map((plan) => <div key={plan.name} className={cn("rounded-3xl p-6", plan.featured ? "bg-white text-zinc-950" : "bg-white/8 text-white")}><div className="flex items-start justify-between gap-4"><div><h3 className="text-2xl font-black">{plan.name}</h3><p className={plan.featured ? "text-zinc-500" : "text-white/55"}>{plan.detail}</p></div>{plan.featured && <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-black text-[#ff5864]">POPULAR</span>}</div><div className="mt-6 flex items-end gap-2"><span className="text-5xl font-black">{plan.price}</span>{plan.price !== "Free" && <span className="pb-2 text-zinc-500">/mo</span>}</div><Button type="button" onClick={() => navigate(plan.featured ? "payment" : "signup")} className={cn("mt-6 h-12 w-full rounded-2xl font-black", plan.featured ? "bg-[#ff5864] text-white hover:bg-[#ff2a4e]" : "bg-white text-zinc-950 hover:bg-white/90")}>{plan.cta}</Button><div className="mt-6 space-y-3">{plan.perks.map((perk) => <div key={perk} className="flex items-center gap-3 text-sm font-semibold"><Icon name="check" className={cn("h-5 w-5", plan.featured ? "text-[#ff5864]" : "text-rose-300")} />{perk}</div>)}</div></div>)}</div></div></section>

      <section id="faq" className="mx-auto max-w-4xl px-6 py-16"><div className="text-center"><p className="text-sm font-black uppercase tracking-[0.25em] text-[#ff5864]">FAQ</p><h2 className="mt-3 text-4xl font-black tracking-tight">Questions about getting started?</h2></div><div className="mt-10 space-y-4">{faqs.map(([question, answer]) => <Card key={question} className="rounded-3xl border-rose-100 bg-white/80 shadow-sm"><CardContent className="p-6"><h3 className="font-black text-zinc-950">{question}</h3><p className="mt-2 leading-7 text-zinc-600">{answer}</p></CardContent></Card>)}</div></section>

      <footer className="mx-auto max-w-7xl px-6 py-10"><div className="flex flex-col items-center justify-between gap-5 rounded-[2rem] bg-white p-6 shadow-sm md:flex-row"><div className="flex items-center gap-3"><LogoMark /><div><div className="font-black">NinjaSwiper</div><div className="text-sm text-zinc-500">© 2026 ninjaswiper.com</div></div></div><div className="flex gap-5 text-sm font-semibold text-zinc-500"><button type="button" onClick={() => navigate("terms")} className="hover:text-[#ff5864]">Terms</button><button type="button" onClick={() => navigate("contact")} className="hover:text-[#ff5864]">Contact</button><a href={CHROME_EXTENSION_URL} target="_blank" rel="noopener noreferrer" className="hover:text-[#ff5864]">Chrome extension ↗</a></div></div></footer>
    </main>
  );
}
